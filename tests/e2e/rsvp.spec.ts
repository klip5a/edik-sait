...
test('reveals conditional fields and submits a complete RSVP', async ({ page }) => {
  ...
  await page.goto('/')
  await page.getByRole('button', { name: '8. Подтверждение присутствия' }).click()
  await page.getByLabel('Ваши имя и фамилия *').fill('Иван Иванов')
  await page.getByRole('radio', { name: 'С удовольствием приду' }).check()

  await page.getByRole('group', { name: 'Будете на церемонии? *' }).getByRole('radio', { name: 'Да' }).check()
  await page.getByRole('group', { name: 'Поедете на фотосессию в парк? *' }).getByRole('radio', { name: 'Да' }).check()
  await page.getByLabel('Сколько вас будет? *').selectOption('2')
  await page.getByRole('group', { name: 'Будете пить алкоголь? *' }).getByRole('radio', { name: 'Да' }).check()
